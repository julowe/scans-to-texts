...
- Each batch directory contains:
  ...
  - Before each page div, a single comment line is inserted stating the overall page number and the source image filename.
...
IMAGE_EXTS = {
    "png", "jpg", "jpeg", "tif", "tiff", "bmp", "gif", "webp", "jp2"
}
...
class HOCRStructure:
    ...
    pages_html: List[str]    # Each entry is a full <div class="ocr_page" ...> ... </div> block
...
def list_images_sorted(images_dir: Path) -> List[Path]:
    files = [p for p in images_dir.iterdir() if p.is_file() and p.suffix.lower().lstrip(".") in IMAGE_EXTS]
...
def extract_hocr_structure(hocr_html: str) -> HOCRStructure:
    ...
    body_open_match = re.search(r"<body\b[^>]*>", hocr_html, flags=re.IGNORECASE | re.DOTALL)
    ...
    if not pages_html:
        raise ValueError("No <div class='ocr_page' ...> elements found in the hOCR file.")

    return HOCRStructure(prefix_before_body=prefix_before_body, body_open_tag=body_open_tag, pages_html=pages_html)
    ...
    while True:
        ...
        while True:
            ...
            if next_close is None and next_open is None:
                raise ValueError("Malformed hOCR: unmatched <div> for an ocr_page block.")
            # Choose the earliest tag
            if next_close is None or (next_open is not None and next_open.start() < next_close.start()):
                depth += 1
                i = next_open.end()
            else:
                depth -= 1
...
def assemble_hocr(prefix_before_body: str, body_open_tag: str, page_blocks_with_comments: Sequence[str]) -> str:
    parts = []
...
def create_batches(
    ...
    def derive_batch_hocr_filename(input_name: str, start_inclusive_zero: int, end_exclusive_zero: int) -> str:
    ...
    for batch_idx, (start, end) in enumerate(batches, start=1):
        ...
        for page_index in range(start, end):
            ...
            comment = f"<!-- overall page {overall_page_num}: source image {image_name} -->\n"
            page_html = hocr.pages_html[page_index]
            page_blocks_with_comments.append(comment + page_html)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Split an hOCR file and source images into proofreading batches.")
    parser.add_argument("--hocr", "-f", type=Path, required=True, help="Path to the input .hocr file")
    parser.add_argument("--images-dir", "-i", type=Path, required=True, help="Path to the directory containing source images")
    parser.add_argument("--output-dir", "-o", type=Path, required=True, help="Directory to create batch subdirectories in")
    parser.add_argument("--batch-size", "-b", type=int, default=20, help="Number of pages/images per batch (default: 20)")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if not images:
        print(f"Error: no supported images found in directory: {images_dir}", file=sys.stderr)
        return 2
    ...
    create_batches(hocr_struct, images, output_dir, batch_size, hocr_basename=hocr_basename)
